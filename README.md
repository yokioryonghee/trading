...
![예시 포트폴리오 성과 그래프](여기에_성과그래프_이미지파일_경로.png)
*(이미지를 포함시키려면: 캡처 파일을 이미지 파일(예: `performance_plot.png`)로 프로젝트 폴더나 `images/` 같은 하위 폴더에 저장한 후, 마크다운 이미지 삽입 문법을 사용하세요: `![이미지 설명](이미지파일_경로.png)`)*
